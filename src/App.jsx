...
const BODY_VB = {"maleFront":"0 0 724 1448","maleBack":"724 0 724 1448","femaleFront":"0 0 724 1448","femaleBack":"724 0 724 1448"};
...
const MEAL_SECTIONS = [
  ...
  { id: "dinner", label: "Dinner" }, { id: "snacks", label: "Snacks" }, { id: "shakes", label: "Protein Shakes" },
];
      ...
      <div style={styles.content}>
        ...
        {tab === "food" && (
          <FoodTab totals={totals} targets={targets} setTargets={updateTargets} todaysEntries={todaysEntries}
            ...
            usdaKey={usdaKey} setUsdaKey={setUsdaKey} />
        )}
  ...
  return (
    <div style={styles.tabContent}>
      ...
      <div style={styles.card}>
        ...
        {sets.map((s, i) => (
          <div key={i}>
            <div style={styles.setRow}>
              ...
              <input style={styles.setInput} type="number" inputMode="decimal" placeholder={lastSession?.sets[i]?.weight || "lbs"} value={s.weight} onChange={(e) => updateSet(i, "weight", e.target.value)} />
              <input style={styles.setInput} type="number" inputMode="numeric" placeholder={ex.r} value={s.reps} onChange={(e) => updateSet(i, "reps", e.target.value)} />
        ...
        <button style={{ ...styles.primaryButton, width: "100%", marginTop: 12 }} onClick={() => onSave(sets)}>{todaySession ? "Update session" : "Save session"}</button>
      </div>
      {sessions.length > 0 && (
        <div style={styles.card}>
...
// ============ FOOD TAB ============
function FoodTab({ totals, targets, setTargets, todaysEntries, addEntry, removeEntry, myFoods, saveCustomFood, usdaKey, setUsdaKey }) {
  ...
  return (
    <div style={styles.tabContent}>
      ...
      {activeAdd && <AddFoodModal sectionLabel={MEAL_SECTIONS.find((s) => s.id === activeAdd)?.label} myFoods={myFoods} usdaKey={usdaKey} setUsdaKey={setUsdaKey} onClose={() => setActiveAdd(null)} onAdd={(entry, save) => { addEntry(activeAdd, entry); if (save) saveCustomFood(entry); setActiveAdd(null); }} />}
    </div>
  );
}
  ...
  return (
    <div style={styles.card}>
      ...
      {["calories", "protein", "carbs", "fat"].map((k) => (
        <div key={k} style={styles.fieldRow}><label style={styles.fieldLabel}>{k === "calories" ? "Calories" : k[0].toUpperCase() + k.slice(1) + " (g)"}</label><input type="number" value={local[k]} onChange={(e) => setLocal({ ...local, [k]: Number(e.target.value) || 0 })} style={styles.input} /></div>
      ))}
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}><button style={styles.primaryButton} onClick={() => { setTargets(local); onClose(); }}>Save</button><button style={styles.secondaryButton} onClick={onClose}>Cancel</button></div>
    </div>
  );
}
function MealSection({ section, items, onAdd, onRemove }) {
  ...
  return (
    <div style={styles.card}>
      ...
      {open && items.length === 0 && <div style={styles.emptyHint}>Nothing logged yet</div>}
    </div>
  );
}

async function lookupBarcode(code, usdaKey) {
  if (!usdaKey) throw new Error("nokey");
  const url = `https://api.nal.usda.gov/fdc/v1/foods/search?api_key=${encodeURIComponent(usdaKey)}&query=${encodeURIComponent(code)}&dataType=Branded&pageSize=25`;
  let res;
...
function BarcodeScanner({ onFound, usdaKey, setUsdaKey, keyInput, setKeyInput }) {
  ...
  async function handleCode(code) {
    ...
    try {
      const food = await lookupBarcode(code, usdaKey);
      if (food) { onFound(food); if (!food.exactMatch) setStatus("Closest branded match (exact UPC not in USDA). Double-check macros."); }
      else setStatus(`Not in USDA branded database (${code}). Use Search or Custom.`);
    } catch (e) {
      if (e.message === "nokey") setStatus("Add your USDA API key below to enable barcode lookup.");
      else if (e.message === "network") setStatus("Can't reach USDA from here. Search and Custom still work.");
      else setStatus(`Lookup error (${e.message}). Try Search or Custom.`);
    } finally { setLooking(false); }
  }
  return (
    <div>
      {!usdaKey && <div style={{ marginBottom: 12 }}><div style={styles.emptyHint}>Barcode lookup uses your USDA key. Paste it once.</div><div style={{ display: "flex", gap: 8 }}><input placeholder="USDA API key" value={keyInput} onChange={(e) => setKeyInput(e.target.value)} style={{ ...styles.input, flex: 1 }} /><button style={styles.primaryButton} onClick={() => setUsdaKey(keyInput)}>Save</button></div></div>}
      <div style={styles.emptyHint}>{supported ? "Type or paste the barcode number below." : "Type the barcode number below — same lookup."}</div>
...
function CustomFoodForm({ onSelect }) {
  ...
  return (
    <div>
      ...
      {["calories", "protein", "carbs", "fat"].map((k) => (<div key={k} style={styles.fieldRow}><label style={styles.fieldLabel}>{k === "calories" ? "Calories" : k[0].toUpperCase() + k.slice(1) + " (g)"}</label><input type="number" style={styles.input} value={form[k]} onChange={(e) => setForm({ ...form, [k]: e.target.value })} /></div>))}
      <button style={styles.primaryButton} onClick={() => { if (!form.name.trim()) return; onSelect({ name: form.name, calories: Number(form.calories) || 0, protein: Number(form.protein) || 0, carbs: Number(form.carbs) || 0, fat: Number(form.fat) || 0, servingNote: "custom entry" }); }}>Continue</button>
    </div>
  );
}
...
function WeightTab({ weights, setWeights, profile, setProfiles }) {
  ...
  function addWeight() {
    if (!val) return; const date = todayKey(); const lbs = Number(val);
...
function BodyTab({ workoutLogs, plans, profile, setProfiles }) {
  ...
  return (
    <div style={styles.tabContent}>
      ...
      <div style={styles.card}>
        ...
        {ranked.length === 0 && <div style={styles.emptyHint}>No working sets logged in this window yet.</div>}
        {ranked.map(([z, v]) => (
