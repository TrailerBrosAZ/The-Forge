...
import { Plus, Trash2, Dumbbell, UtensilsCrossed, Scale, ChevronDown, ChevronUp, X, ChevronLeft, ChevronRight, Trophy, History, Check, Library, Pencil, ArrowLeft, LayoutDashboard, PersonStanding } from "lucide-react";
...
// ============ FOOD TAB ============
function FoodTab({ totals, targets, setTargets, todaysEntries, addEntry, removeEntry, myFoods, saveCustomFood, deleteFood }) {
  ...
  return (
    <div style={styles.tabContent}>
      ...
      {activeAdd && <AddFoodModal sectionLabel={MEAL_SECTIONS.find((s) => s.id === activeAdd)?.label} myFoods={myFoods} deleteFood={deleteFood} onClose={() => setActiveAdd(null)} onAdd={(entry, save) => { addEntry(activeAdd, entry); if (save) saveCustomFood(entry); setActiveAdd(null); }} onSaveRecipe={(recipe) => { saveCustomFood(recipe); }} />}
    </div>
  );
}
...
function AddFoodModal({ sectionLabel, myFoods, deleteFood, onClose, onAdd, onSaveRecipe }) {
  const [mode, setMode] = useState(myFoods.length > 0 ? "myfoods" : "custom");
    ...
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={styles.modalSheet} onClick={(e) => e.stopPropagation()}>
        ...
        {selected ? (
          ...
          <>
            <div style={styles.modeSwitch}>{["myfoods", "custom", "recipe"].map((m) => (<button key={m} style={{ ...styles.modeButton, ...(mode === m ? styles.modeButtonActive : {}) }} onClick={() => setMode(m)}>{m === "myfoods" ? "My Foods" : m === "custom" ? "Custom" : "Recipe"}</button>))}</div>
            {mode === "myfoods" && (
